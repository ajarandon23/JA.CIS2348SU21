...
# 4. finally a new list is created for past service date items for the final inventory list
date_list = []
    ...
    for items in new_date_list:
        for x in range(len(new_manufactlist)):
            if items[0] == new_manufactlist[x][0]:
                date_list.append(
                    [new_manufactlist[x][0], new_manufactlist[x][1],
                     new_manufactlist[x][2], new_manufactlist[x][3],
                     new_manufactlist[x][4], new_manufactlist[x][5]])
...
# here a for loop is created to remove all past-service items from the final list
# will create a final list which will contain all of the non-expired items
# if item is not located in the past-service file, it will be appended to the final list
f_gold_list = []
for item in final_list:
    if item not in date_list:
        f_gold_list.append(item)

final_gold_list = []
for item in f_gold_list:
    if item not in damaged_product_list:
        final_gold_list.append(item)
...
# here letter Q is set as the exit value to terminate the program
# if q is not entered the program with continue to execute
# matched/non-matched items will be appended to xy and yz lists
while input_manufacturer != "q":
    xy_item = []
    yz_item = []
    for z in range(0, len(final_gold_list)):
        if input_manufacturer in final_gold_list[z][1] and input_type in final_gold_list[z]:
            xy_item.append(final_gold_list[z])
        else:
            if input_type in final_gold_list[z]:
                yz_item.append(final_gold_list[z])

    # here if list remains unchanged, product does not exist
    if len(xy_item) != 0:
        xy_item = sorted(xy_item, key=itemgetter(4), reverse=True)
        print("\nYour Item is: ", xy_item[0])
    else:
        print("No such item in inventory")

    if len(yz_item) != 0:
        yz_item = sorted(yz_item, key=itemgetter(4), reverse=True)
        print("You may also consider: ", yz_item[0])

# option to continue executing program or exit program
input_manufacturer = str(input("\nEnter your manufacturer, or q to exit query: "))
